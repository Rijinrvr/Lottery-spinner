...
const { FINAL_NUMBERS, WIN_MESSAGE_DELAY, SLOT_COUNT } = LOTTERY_CONFIG;

const LotterySpin: React.FC = () => {
  const [showWinMessage, setShowWinMessage] = useState(false);

  const handleSpinComplete = (_finalNumbers: number[]) => {
    setTimeout(() => {
  ...
  const { isSpinning, displayNumbers, startSpin, cleanup } = useLotterySpin({
    initialNumbers: [...FINAL_NUMBERS],
    onComplete: handleSpinComplete,
});


  const handleSpin = () => {
    const randomNumbers = Array.from({ length: SLOT_COUNT }, () =>
      Math.floor(Math.random() * 10)
    );
    setShowWinMessage(false);
    startSpin(randomNumbers);
  };
